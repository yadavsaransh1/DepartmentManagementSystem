...
import './TeacherMyDocuments.css';

const TeacherMyDocuments = () => {
  ...
  // Document categories - Academic documents only (admin-uploaded documents)
  ...
  const fetchMyDocuments = async () => {
    try {
      setLoading(true);
      setError('');
      // Fetch documents accessible to the logged-in teacher
      const response = await api.get('/documents/search');
      const docs = Array.isArray(response.data) ? response.data : response.data?.content || [];
      console.log('Teacher My Documents fetched:', docs);
      setDocuments(docs);
  ...
  const filterDocuments = (docs, search) => {
    ...
    // Filter by academic documents only (admin-uploaded documents: CERTIFICATE, RESULT, IDENTITY, ADMISSION)
...
export default TeacherMyDocuments;
